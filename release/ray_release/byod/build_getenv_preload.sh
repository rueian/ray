...
# Don't preload during build: the .so doesn't exist yet. LD_PRELOAD is for runtime only.
unset LD_PRELOAD
...
gcc -shared -fPIC -o getenv_trace_preload.so getenv_preload.c -ldl -lpthread -Wall -Wno-nonnull-compare
