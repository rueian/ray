...
def _create_project_ssh_key_pair(project, public_key, ssh_user, compute):
    ...
    except errors.HttpError as e:  # we only remove ssh keys in CI for not suprising normal users.
        if e.resp.status != 413 or ssh_keys_i is None or not os.environ.get("CI"):
            raise
        logger.warning(
